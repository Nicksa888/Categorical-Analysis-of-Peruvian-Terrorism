...
rm(list = ls()) 
# Remove all loaded data sets, functions and so on.
...
library(easypackages) # enables libraries function for easier library loading
suppressPackageStartupMessages(
  libraries("data.table", # for set.names function
            "dplyr",
            "BayesFactor", # for contingencyTableBF()
            "ggplot2",
...
# Some cities were incorrectly inputted, so they have been changed as follows
...
Peru_Raw <- Peru_Raw %>% relocate( # move lethal before dead variable
  Lethal, 
  .before = Dead
)
...
#########################################
# Bayes Factor Decade Feature Selection #
#########################################

DBFPV <- setdiff(colnames(Peru_DVS), "Lethal") # Remove Lethal from predictor variables

DBFPV_nlevels <- sapply(Peru_DVS[, DBFPV], function(i)length(unique(i))) 
# ensure all variables have at least one level
DBFPV <- DBFPV[DBFPV_nlevels > 1]
length(DBFPV)
results <- lapply(Peru_DVS[, DBFPV], function(i){
  CT <- table(i, Peru_DVS$Lethal)
  BA <- contingencyTableBF(CT, sampleType = "poisson") 
  # my sample is random, so poisson is suitable 
  list(CT = CT, BA = BA)
})
...
##########################################################
# Data featuring Bayesfactor scores for Decade Variables #
##########################################################
...
# To determine the city variables
...
#######################################
# Bayes Factor City Feature Selection #
#######################################
...
########################################################
# Data featuring Bayesfactor scores for City Variables #
########################################################
...
# Identify province variable numbers

which(colnames(Peru_FC) == "Lethal") # 
which(colnames(Peru_FC) == "ProvincesStartHere") # 
which(colnames(Peru_FC) == "CitiesStartHere") # 

# To determine province variables
...
###########################################
# Bayes Factor Province Feature Selection #
###########################################
...
############################################################
# Data featuring Bayesfactor scores for Province Variables #
############################################################
...
# To determine the target variables
...
#########################################
# Bayes Factor Target Feature Selection #
#########################################
...
# To determine the group variables
...
########################################
# Bayes Factor Group Feature Selection #
########################################
...
# To determine the attack tyep variables
...
##############################################
# Bayes Factor Attack Type Feature Selection #
##############################################
...
###############################################################
# Data featuring Bayesfactor scores for Attack Type Variables #
###############################################################
...
# Final Attack Type Variable Selected List 
...
# To determine the weapon variables
...
#########################################
# Bayes Factor Weapon Feature Selection #
#########################################
...
###############################################################
# Data featuring Bayesfactor scores for Weapon Type Variables #
###############################################################
...
# This section indicates the variables selected by BayesFactor #

##############################
# Final Province Variable List
##############################

# Lima,	Ayacucho, Huancavelica, Apurimac, Huanuco, Puno, SanMartin 

############################
# Final Target Variable List
############################

# GovernmentDiplomatic, Business, Food Water Supply, Utilities, JournalistsMedia,  Private, Transportation, Police, TerroristsNonStateMilitia

###########################
# Final Group Variable List
###########################

# Shining Path and Tupac Amaru Revolutionary Movement  

#############################
# Final Weapons Variable List
#############################

# Explosives, Firearms, Incendiary, Melee

#################################
# Final Attack Type Variable List
#################################
...
###############################################################
# Data featuring Bayesfactor scores for Final Model Variables #
###############################################################
